...
exports.userResetpasswordMail = async (request, response, next) => {
    try {
        ...
        if (user) {
            const sender = {
                email: 'forgotpassword@chatapp.com',
                name: 'Admin'
            }
            const receivers = [
                {
...
exports.userResetpasswordform = async (request, response, next) => {
    try {
        ...
        if (passwordreset.isactive) {
            ...
            return response.status(401).json({ message: "Link has expired, Please generate a new one." })
        }
    ...
    try {
        ...
        if(timeDifference <= timeLimit){
          ...
          await User.update(
            ...
            response.status(403).json({ message: "Link has expired, Please generate a new one."});
        }
